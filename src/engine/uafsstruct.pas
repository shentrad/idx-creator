...
type
  ...
  TAfsStruct = class
    ...
    public
      ...
      property Items[Index: Integer]: TAfsEntry read GetItem; Default;
...
begin
  ...
  if strBuf = AFS_SIGN then begin
    ...
    //Parsing all the files info
    for i:=0 to fCount-1 do begin
      AfsEntry := TAfsEntry.Create;

      //Reading offset and size of the file
      Seek(F, 8+(8*i));
      ...
      if lOffset <> -1 then begin
        //Reading informations from the list
        Seek(F, lOffset+(48*i));
        SetLength(strBuf, 32);
        ...
        //If there's no files list, we store generic infos...
